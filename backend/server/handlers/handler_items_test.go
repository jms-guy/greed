...
func TestHandlerProcessWebhookRecords(t *testing.T) {
	tests := []struct {
		name            string
		userIDInContext uuid.UUID
		requestBody     string
		...
		{
			name:            "should successfully process webhook records",
			userIDInContext: testUserID,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest("DELETE", "/api/items/webhook-records", bytes.NewBufferString(tt.requestBody))
			req.Header.Set("Content-Type", "application/json")
			...
			mockApp.HandlerProcessWebhookRecords(rr, req)
